...
static uint8_t current_pr2 = 0;

static uint8_t pwm_pin_to_ccp(uint8_t pin) {
...
static void pwm_ccp_init(uint8_t ccp) {
    switch (ccp) {
...
static void pwm_ccp_set_duty(uint8_t ccp, uint8_t duty) {
    uint16_t period = (uint16_t)4 * (uint16_t)(current_pr2 + 1);
...
static void pwm_timer2_freq(unsigned long freq_hz) {
